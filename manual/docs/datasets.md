...
1. Invoke a user defined function decorated with `@dataset_load`.
    - If there is no function or the section is disabled in the config file (i.e. `enable=False`),


!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    this step has failed, so `Log(ML)` will attempt to create a dataset (next step)
...
# DataFrames
Dataset load default implementation for `dataset_type='df'` (i.e. DataFrame) reads a dataFrame from a CSV file using `pandas.read_csv`
